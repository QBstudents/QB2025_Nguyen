...
> ***Answer 1a***: There are 4 objects in the `doubs` dataset. They are `env`, `fish`, `xy`, `species`
...
> ***Answer 2a***: We can see that there is a big difference between species richness between sites. For instant, sites 27->30 have more than 20 different kinds of fishes (similar to site 17 -> 22). While other sites have very few species.
> Site 8 does not have any fishes.

> ***Answer 2b***:  The Brown Trout is most abundant for some sites (up to 5 encountered). While for some other sites, the Brown Trout is almost absent. 
```{r}
# 2b
doubs$fish$Satr
```
> I will also plot out the distribution of Brown Trout accross sites
```{r}
# Plot doubs$fish$Satr over x, y coordinates
plot(x=doubs$xy$x,
    y=doubs$xy$y,
    xlab="X coordinates in km",
    ylab="Y-coordinates in km",
    col=sort(unique(doubs$fish$Satr)),
    main="Brown Trout abundance in the Doubs River",
    pch=19, 
    cex=5,
    # add number of sites to the dots,
    labels=doubs$sites$Site)

legend("topright", legend=unique(doubs$fish$Satr), col=sort(unique(doubs$fish$Satr)), pch=19)
# Background grid
grid()

```
> ***Answer 2c***:  
> There is a difference between species richness and the abundance of a specific species in accross datasets. 
